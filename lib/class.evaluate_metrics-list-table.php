...
class Evaluate_Metrics_List_Table extends WP_List_Table {
	...
	function column_default( $item, $column ) {
		switch ( $column ):
			case 'nicename':
				$name_link = sprintf( '<a href="?page=evaluate&view=metric&metric_id=%s"><b>%s</b></a>', $item->id, wp_unslash( $item->nicename ) );
				$row_actions = array(
					...
					'delete' => sprintf( '<span class="trash"><a href="?page=evaluate&eval_action=delete&metric_id=%s&_wpnonce=%s"  onclick="return Evaluate_Admin.confirmDeletion();">Delete</a></span>', $item->id, wp_create_nonce( 'evaluate-delete-' . $item->id ) ),
				);
